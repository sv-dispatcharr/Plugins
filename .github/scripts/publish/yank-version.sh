...
: "${GITHUB_REPOSITORY:?}" "${GITHUB_TOKEN:?}" "${YANK_PLUGIN:?}" "${YANK_VERSION:?}" "${SOURCE_BRANCH:?}"
...
echo "Yanking $YANK_PLUGIN v$YANK_VERSION from $RELEASES_BRANCH"
...
> Opened automatically by the Yank Plugin Version workflow."
  else
    ...
    if [[ "$SOURCE_TYPE" == "local" ]]; then
      ...
      if [[ -n "$RESTORE_COMMIT" ]]; then
        ...
        SOURCE_NOTE="Plugin source files restored from commit \`${RESTORE_COMMIT:0:7}\` (the last commit where \`${YANK_PLUGIN}\` was at v\`${NEW_LATEST_VERSION}\`)."
      else
        ...
        SOURCE_NOTE="⚠️ **Could not restore source files** — no commit found for v\`${NEW_LATEST_VERSION}\` (history may have been squashed). Only the \`version\` field was updated. Please review the source files manually before merging."
      fi
    else
...
\`${YANK_PLUGIN}\` v\`${YANK_VERSION}\` has been yanked from the releases branch. This PR rolls the source back to v\`${NEW_LATEST_VERSION}\` so the yanked version is not republished on the next run.
...
> Opened automatically by the Yank Plugin Version workflow. Review and merge to complete the rollback."
  fi
  ...
  echo "Rollback PR opened: $ROLLBACK_BRANCH → $SOURCE_BRANCH"
fi
