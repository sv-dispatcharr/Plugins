...
**Automating version bumps with Renovate:**

Add a `regexManagers` entry to your Renovate config to bump the `version` field in `plugin.json` automatically. Each version-bump PR still requires maintainer approval before anything is published.
